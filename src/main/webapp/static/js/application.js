    ...
    initCommonBtn : function() {
        $(".btn-view-info,.btn-change-password").click(function() {
            ...
            if(a.is(".btn-view-info")) {
                url = ctx +"/admin/sys/user/common/viewInfo";
            } else if(a.is(".btn-change-password")) {
                url = ctx + "/admin/sys/user/common/changePassword";
            }
            setTimeout(function() {
...
$.table = {
    ...
    initSort: function (table) {
        ...
        table.find("[sort]").each(function () {
            ...
            var sortPropertyName = prefix + "page.sort=" + th.attr("sort")+"&page.direction";
            var sortBtnStr = null;
    ...
    removeSortParam : function(sortURL) {
        sortURL = sortURL.replace(/\&\w*page.sort=.*?&page.direction=((asc)|(desc))/gi, '');
        sortURL = sortURL.replace(/\?\w*page.sort=.*?&page.direction=((asc)|(desc))\&/gi, '?');
        sortURL = sortURL.replace(/\?\w*page.sort=.*?&page.direction=((asc)|(desc))/gi, '');
        return sortURL;
    },
    ...
    initUpdateSelected : function($table, urlPrefix) {
        ...
        $btn.off("click").on("click", function() {
            ...
            if(!checkbox.length)  return;
            var id = checkbox.val();
            window.location.href = urlPrefix +  "/update?id="+id+"&BackURL=" + $.table.encodeTableURL($table);
        });
    },
