    ...
    initMovableTree : function(config) {
        ...
        var setting = {
            ...
            edit: {
                ...
                showRenameBtn: config.permission.rename,
                removeTitle: "移除",
                renameTitle: "重命名",
                drag : {
        ...
        function onClick(event, treeId, treeNode, clickFlag){
        	...
        	if(config.clickUrl == ''){config.clickUrl = config.detialsUrl.replace("{id}", treeNode.id);}
        	config.obj.location.href=config.clickUrl;
        	config.clickUrl = '';
        }
        ...
        var detialsStr = "<span class='btn_file' id='detialsBtn_" + treeNode.id
            + "' title='查看子节点' onfocus='this.blur();'></span>";
        sObj.after(detialsStr);
        var btn2 = $("#detialsBtn_" + treeNode.id);
        if (btn2)
            btn2.bind("click", function (e) {
                onDetials(e, treeId, treeNode);
            	
                return false;
            });
        if(treeNode.root) return;
        var updateStr = "<span class='btn_tool' id='updateBtn_" + treeNode.id
        + "' title='修改子节点' onfocus='this.blur();'></span>";
        sObj.after(updateStr);
        var btn3 = $("#updateBtn_" + treeNode.id);
        if (btn3)
        btn3.bind("click", function (e) {
            onUpdate(e, treeId, treeNode);
        ...
        function removeHoverDom(treeId, treeNode) {
            ...
            $("#detialsBtn_" + treeNode.id).unbind().remove();
            $("#updateBtn_" + treeNode.id).unbind().remove();
        }
        function onUpdate(e, treeId, treeNode){
        	 var url = config.updateUrl.replace("{id}", treeNode.id);
        	 config.clickUrl = url;
             $("#" + newNode.tId + "_a").click();
        }
        
        function onDetials(e, treeId, treeNode){
        	 var url = config.detialsUrl.replace("{id}", treeNode.id);
        	 config.clickUrl = url;
             $("#" + newNode.tId + "_a").click();
        ...
        function onRename(e, treeId, treeNode) {
            ...
            $.post(getUrl(url),getData(url),function(data){
            	  $.app.waitingOver();
            	},"json"
            	);
...
//            });
    ...
    initSelectTree : function(config) {
        ...
        function checkSelect(zTree,ids) {
        	if(ids==null){
        		return;
        	}
        	var array=ids.split(",");
        	var nodes = zTree.getNodes();
        	var zNodes =zTree.transformToArray(zTree.getNodes());
        	for(var i=0;i<zNodes.length;i++){
        		
        		if(jQuery.inArray(zNodes[i].id+'', array)>=0){
        			zTree.checkNode(zNodes[i],true,false);
        			$name.prop("value", fullName(zNodes[i]));
        		}
        	}
        }
        ...
        var initTree = function() {
            ...
            if(config.checkSelect){
            	checkSelect(zTree,config.selectIds);
            }
            if(autocomplateEnable) {
                if(!config.autocomplete.minLength) config.autocomplete.minLength = 0;
                config.autocomplete.enterSearch = true;
